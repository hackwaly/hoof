(function () { 'use strict';
        ...
        context.bindings.push(bind(script.property, function (array) {
            ...
            for (var i = 0; i < allocQueue.length; i++) {
                ...
                if (mapping === null) {
                    ...
                    var _context = render(mappedScript);
                    mapping = {
                        _context: _context,
    ...
    function waitForNextReflow(callback) {
    ...
    var databases = stored(ENV.generateData().toArray());
    var template = React.createElement("table", { class: "table table-striped latest-data" }, React.createElement("tbody", { "data-bind": "foreach: databases" }, dynamicList(databases, function (db) {
        var dbname = computed(function () {
            return db().dbname;
        });
        var nbQueries = computed(function () {
            return db().lastSample.nbQueries;
        });
        var countClassName = computed(function () {
            return db().lastSample.countClassName;
        });
        var topFiveQueries = computed(function () {
            return db().lastSample.topFiveQueries;
        });
        return React.createElement("tr", null, React.createElement("td", { class: "dbname" }, dbname), React.createElement("td", { class: "query-count" }, React.createElement("span", { class: countClassName }, nbQueries)), dynamicList(topFiveQueries, function (query) {
            var text = computed(function () {
                return query().query;
            });
            var formatElapsed = computed(function () {
                return query().formatElapsed;
            });
            var elapsedClassName = computed(function () {
                return query().elapsedClassName;
            });
            return React.createElement("td", { class: elapsedClassName }, formatElapsed, React.createElement("div", { class: "popover left" }, React.createElement("div", { class: "popover-content" }, text), React.createElement("div", { class: "arrow" })));
        }));
    })));
    var ctx = render(template);
